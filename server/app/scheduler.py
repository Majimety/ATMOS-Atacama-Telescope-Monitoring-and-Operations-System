...
class ObservationJob:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "progress_pct": round(
                min(100, self.elapsed_s / max(1, self.duration_s) * 100), 1
            ),
...
class ObservationScheduler:
    def __init__(self):
        ...
        self._running = False
        self._lock: asyncio.Lock | None = None  # สร้างใน event loop — ไม่ใช่ตอน import
    ...
    def _get_lock(self) -> asyncio.Lock:
        """Lazy-create Lock ใน event loop ที่กำลังทำงาน (Python 3.10+ safe)"""
    ...
    def _seed_demo_queue(self):
        demo = [
            ObservationJob(
                ...
                notes="Galactic centre monitoring",
            ),
            ObservationJob(
                "M87",
                "12h30m49s",
                ...
                notes="EHT follow-up, B3 continuum",
            ),
            ObservationJob(
                "Orion KL",
                "05h35m14s",
                ...
                1800,
                priority=JobPriority.NORMAL,
                notes="Hot core chemistry survey",
            ),
            ObservationJob(
                "3C 273",
                "12h29m06s",
            ...
            ObservationJob(
                ...
                notes="Pulsar timing — needs low PWV",
            ),
        ]
        self._queue = demo
    ...
    async def move_job(self, job_id: str, direction: int):
        ...
        async with self._get_lock():
            idx = next(
                (i for i, j in enumerate(self._queue) if j.job_id == job_id), None
            )
            if idx is None:
                return
    ...
    def _can_observe(self, job: ObservationJob) -> tuple[bool, Optional[str]]:
        ...
        if job.el < job.min_el_deg:
            return (
                False,
                f"Target elevation {job.el:.1f}° < minimum {job.min_el_deg:.1f}°",
            )
        if self._last_pwv > job.max_pwv_mm:
            return (
                False,
                f"PWV {self._last_pwv:.2f} mm > maximum {job.max_pwv_mm:.2f} mm",
            )
    ...
    async def tick(self, snapshot: dict):
        ...
        async with self._get_lock():
            # Advance active job
            if self._active:
                self._active.elapsed_s = time.time() - (
                    self._active.started_at or time.time()
                )
    ...
    def get_state(self) -> dict:
        return {
            ...
            "stats": {
                ...
                "skipped": sum(
                    1 for j in self._history if j.status == JobStatus.SKIPPED
                ),
            },
        }
